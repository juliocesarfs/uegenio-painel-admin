...
import { VendaClientValidacaoTipo } from './venda-nome';
...
export class VendaProdutoComponent extends  AbstractComponent implements OnInit {
  public canvg;
  ...
  public ticketMedio;
  public tipo:any[];
  public tipoProduto: VendaClientValidacaoTipo[];
  public tipoProduto1: VendaClientValidacaoTipo[];
  public tipoNome:any;
  public reduced:any[]=[];
...
*/
  ...
  public valor(){
    this.valorVenda=0;
    for(this.i=0;this.i<this.reduced.length;this.i++){
      this.valorVenda= this.valorVenda +Number(this.reduced[this.i].quantidade);
    }
  }
  ...
  public getValor(){
    ...
    let cont= this.objetosFiltrados.map(e=>e.id);
    this.tipo=this.objetosFiltrados.map(e=>e.itemVenda);


    console.log("Nome tipo", this.tipo);


    for(i=0;i<this.objetosFiltrados.length;i++){

       for(j=0;j<this.objetosFiltrados.length;j++){

      if(this.tipo[j][0].tipoProduto!==undefined && this.tipo[i][0].tipoProduto!== undefined){

       if(this.tipo[j][0].tipoProduto==this.tipo[i][0].tipoProduto){

         this.valorVenda=this.valorVenda+Number(this.tipo[j][0].quantidadeVendida);
         this.tipoNome=this.tipo[i][0].tipoProduto;
       }}
    ...
    this.tipoProduto.push({
      tipo: this.tipoNome,
      quantidade: this.valorVenda,});


    this.valorVenda=0;
  }

  console.log("Opaa", this.tipoProduto);
  ...
  valorCerto() {
    ...
    this.tipoProduto.forEach((item) => {
    var duplicated  = reduced.findIndex(redItem => {
        return item.tipo == redItem.tipo;
    }) > -1;
  ...
  this.reduced= reduced.concat(this.reduced);
...
public converteData(DataDDMMYY) {
  ...
  public dadosVenda(){
    ...
    let dataFinal = this.converteData(this.vendaDataFinal);
    this.objetosFiltrados = this.vendaPorcentagemCliente.filter(result =>
      {
