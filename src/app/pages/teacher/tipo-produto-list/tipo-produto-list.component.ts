...
export class TeacherListComponent extends AbstractComponent implements OnInit {

  public filtroDTO: FiltroTeacherDTO;
   ...
   */
  ngOnInit() {
    this.filtroDTO = FiltroTeacherDTO.getInstace();
   ...
   */
  public pesquisar(filtroTeacherDTO: FiltroTeacherDTO): void {
    this.teacherClientService.getByFiltro(filtroTeacherDTO).subscribe(data => {
      this.dataSource.paginator = this.paginator;
  ...
  private remover(usuario: any): void {
    this.messageService.addConfirmYesNo('MSG045', () => {
      this.teacherClientService.remover(usuario).subscribe(() => {
